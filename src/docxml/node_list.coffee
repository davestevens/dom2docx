  ...
  class DocXMLNodeList
    ...
    convert_node: (xml) ->
      switch @node.nodeName
        ...
        when "B", "STRONG" then xml.bold(@_traverse)
        when "I", "EM" then xml.italic(@_traverse)
        when "U", "INS" then xml.underline(@_traverse)
        when "SMALL" then xml.small(@_traverse)
        when "STRIKE", "DEL" then xml.strike(@_traverse)
